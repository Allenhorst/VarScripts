...
param (
    <#[Parameter(Mandatory=$true)]#> [String] $oldDeskId = "60",
    <#[Parameter(Mandatory=$true)]#> [String] $newDeskId = "60",
    <#[Parameter(Mandatory=$true)]#> [String] $esxHost = "srv1107.paragon-software.com",
    [String] $vcenter = "vcenter-dlg-prm",
#    [String] $oldDeskId = "66",
#    [String] $newDeskId = "99",
    [String] $esxHost2 = "srv405",
...
if($runDirectly){
    switch($runCommand){
        "runAddToInventory" {
			Connect-VIServer -Server vcenter-dlg-prm -User paragon\saakyan	-Password nDds4fq8
			#Connect-VIServer -Server srv821 -User autotester	-Password  asdF5hh
			$h=Get-VMHost srv042*
        ...
        "runMovingToDS" {
           ...
           <#   #  "ta-$esxHostShort" = "hdd1"; #>   
            ...
            $vmList|%{
                ...
                $vmLocationName = ("$esxHost2`:" + $dsLocationTable[($dsLocationTable.Keys|?{$vmName.Replace("-$newDeskId","") -like $_})])
                
                $dsList > c:\tmp\dslist.txt
