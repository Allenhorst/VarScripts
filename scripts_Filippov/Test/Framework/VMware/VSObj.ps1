function Create-VServer{
    ...
    $VServer | Add-Member -MemberType ScriptMethod -Name GetVMHostLogs -Value {
        param (
                $logFolder
        )
        $this.Connect()
        try
        {
            ...
            $this.Log("Saving logs from VMHost $vmHostName to $logFolder is started")
            $vmHost | get-log -Bundle -Destination "$logFolder"
            $this.Log("Saving logs from VMHost $vmHostName to $logFolder is completed")
        }
        catch
        {
            $this.Log("Errors occurred VsObj.GetVMHostLogs:`n" + $Error)
            $this.Disconnect()
            return $false
        }
        $this.Disconnect()
    ...
    $VServer | Add-Member -MemberType ScriptMethod -Name ClearVCPoolByAdress -Value {
        ...
        iF($pool){
            ...
            if($agentId -and $VMs)
            {
                $VMs = $VMs | where {$_.Name -match ("-" + "$agentId" + "_")}
            }
            If($VMs){
                $VMs | % {
    ...
    $VServer | Add-Member -MemberType ScriptMethod -Name CopyToGuest -Value {
        ...
        if($_vm){
            if(Test-Path $source){
                try
                {
                    while(($isCopyFine -eq $false) -and ($tryNumber -le $tryLimit))
                    {
                        $isCopyFine = $true
                        
                        $this.Log("Copy file [$source] to VM $vmName to folder [$destination] try number $tryNumber")
                        $Error.Clear()
                        Copy-VMGuestFile -Source $source -Destination $destination -VM $_vm -LocalToGuest `
                                         -GuestUser $this.GuestUser -GuestPassword $this.GuestPassword `
                                         -Confirm:$false -Force:$true  | Out-Null
                        
                        if($Error[0] -ne $null)
                        {
                            if($Error[0] -match "Copy-VMGuestFile")
                            {
                                $isCopyFine = $false
                                $this.Log("Copy file [$source] to VM $vmName to folder [$destination] try number $tryNumber failed")
                                $this.Log("Errors occurred:`n" + $Error[0])
                                start-sleep $tryPauseSeconds
                                $tryNumber += 1
                            }
                        }
                    }
                    if($isCopyFine -eq $true)
                    {
                        $this.Log("Copy file [$source] to VM $vmName to folder [$destination] try number $tryNumber succeeded")
                    }
                }catch{$this.Log("Copy file [$source] to VM $vmName to folder [$destination] failed" + $Error[0])}
            }
    ...
    $VServer | Add-Member -MemberType ScriptMethod -Name MountNFSVolume -Value {
        Param
        (
            [string] $name,
            [string] $remoteHost,
        ...
        try
        {
            New-Datastore -Name $name -VMHost $this.Name -Nfs:$true -NfsHost $remoteHost -Path $remotePath
        }
        catch{$this.Log("Errors occurred:`n" + $Error)}
        $this.Disconnect()
    }
    
    $VServer | Add-Member -MemberType ScriptMethod -Name UnmountNFSVolume -Value {
        Param
        (
            [string] $name
        )
        ...
        try
        {
            Remove-Datastore -Datastore (Get-Datastore -Name $name) -Confirm:$false 
        }
        catch{$this.Log("Errors occurred:`n" + $Error)}
        $this.Disconnect()
    }
    ...
    $VServer | Add-Member -MemberType ScriptMethod -Name AddPrefixToLastVMSnapshot -Value {
        Param
        (
            [string] $vmName,
        ...
        $this.Connect()
        $this.Log("Rename last snapshot for $vmName")
        $_vm = $null
        if(($vmName  -ne $null) -and ($vmName  -ne "") -and ($vmName  -ne " ")){ $_vm = (Get-VM -Name $vmName -ErrorAction:SilentlyContinue) }
        if($_vm){
            try{
                $snapshotList = [array](Get-Snapshot -VM $_vm)
                $snapshotList = $snapshotList | sort -property Created
                $snapshot = $snapshotList[-1]
                $newName = $prefix + $snapshot.Name
                $newDescription = $prefix + $snapshot.Description
                $changedSnapshot = Set-Snapshot $snapshot -Name "$newName" -Description "$newDescription" -Confirm:$false
                $this.Disconnect()
                if(($changedSnapshot.Name -eq $newName) -and ($changedSnapshot.Description -eq $newDescription))
                {
                    $this.Log("snapshot was renamed to $newName with description $newDescription successfully")
                    return $true
                }
                else
                {
                    $this.Log("Error! snapshot was NOT renamed to $newName with description $newDescription")
                    return $false
                }
            }
            catch{ $this.Log("Errors occurred:`n" + $Error) }
        ...
        $VServer | Add-Member -MemberType ScriptMethod -Name RemoveLastVMSnapshot -Value {
        Param
        (
            [string] $vmName,
            [string] $prefix = "renamed_"
        ...
        $this.Connect()
        $this.Log("Remove last snapshot for $vmName")
        $_vm = $null
        if(($vmName  -ne $null) -and ($vmName  -ne "") -and ($vmName  -ne " ")){ $_vm = (Get-VM -Name $vmName -ErrorAction:SilentlyContinue) }
        if($_vm){
            try{
                ...
                if($snapshotName -match "$prefix")
                {
                    $null = Remove-Snapshot -Snapshot $snapshot -Confirm:$false
                }
                $this.Disconnect()
                return $true
            }
