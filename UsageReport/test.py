...
if reports == [] :
    ...
    text = Reporter.Reporter.generateReport(Reporter.Reporter, tree, max_dLevel, max_wLevel)
    ...
    TM.message("find report")
    lastReport = findLastFile(reports)
    name = lastReport[-24:-5]
    TM.message("load it")
    L1 = loader.Loader(lastReport)
    TM.message("parse report")
    js = L1.parseJson(lastReport)
    loader.Loader.tree= treelib.Tree()
    TM.message("build report tree")
    old = loader.Loader.addProjectNode(loader.Loader, jsondata=js)
    #builded tree from existing report
    TM.message("get smart build size")
    DiskUsage.DiskUsage.getSmartAllBuildsArtsize(DiskUsage.DiskUsage, rootProjectId, info=old, date=name)
    TM.message("build tree")
    tree = DiskUsage.DiskUsage.buildCustomTree(DiskUsage.DiskUsage, rootProjectId)
    TM.message("write json")
    DiskUsage.DiskUsage.projectToJSON(DiskUsage.DiskUsage, rootProjectId)
    DiskUsage.DiskUsage.closeFile(DiskUsage.DiskUsage)
    #L = loader.Loader(DiskUsage.DiskUsage.path)
    #js1 = L.parseJson(DiskUsage.DiskUsage.path)
    ...
    TM.message("generate report")
    text = Reporter.Reporter.generateCompReport(Reporter.Reporter, tree, old, max_dLevel, max_wLevel)
